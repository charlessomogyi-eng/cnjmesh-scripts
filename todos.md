...
- **[URGENT] Watchdog / auto-heal for `meshcore-mqtt-bridge`.** Priority item — it fails **silently** and, critically, on your **own routine maintenance**: any Mosquitto/cnjmesh1 restart or update severs the bridge's socket, it fills its 1000-msg queue (`MQTT: stopped | Queue: 1000/1000`, `Timeout queueing message`) and never reconnects, killing the MeshCore Hub feed until a manual `docker restart meshcore-mqtt-bridge`. Fix, in order of preference: (1) MQTT reconnect/keepalive — check `/opt/meshcore-mqtt/.env.docker` for a reconnect setting, or whether `meshcore-mqtt:local` supports auto-reconnect at all (best fix if it exists); (2) failing that, deploy a **watchdog on cnjmesh3** (same pattern as the CoreScope ingest-stall + graywolf watchdogs) that greps `docker logs --tail N meshcore-mqtt-bridge` for `MQTT: stopped` / queue pinned at 1000 and issues `docker restart`. Healthy signature: `MQTT: connected | Queue: 0/1000 | Dropped: 0`. Also confirm the container has `restart: unless-stopped` (covers the die case, not this deaf case).
