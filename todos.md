...
- **[NEW Aug 26]** Send an actual addressed APRS message (not just an automatic position beacon) from K2GIA-9 to K2GIA-10 to confirm messaging specifically, not just beaconing/digipeating (already confirmed working). Low priority — high confidence it'll work given everything else tested clean, just not directly proven yet.
...
- **[NEW Aug 26]** Try Winlink over RF as a real-world test, per Compy (friend, runs a Winlink gateway + BBS on VHF). Install Winlink Express (Windows PC first, per Compy's advice), connect via Packet or VARA modem. Compy's own gateway is AC2NJ-10 (145.010 MHz, 1200 baud) but he noted Charles likely has a closer gateway — use Winlink Express's Packet Channel Selector to scan and pick the nearest one instead of reaching all the way to Compy's (his antenna is only 135ft, better to test from a rooftop if reaching for his specifically).
