...
- **[NEW Aug 18 — active investigation]** CentralNJ MQTT gap — confirmed real: general broker traffic (LongFast, CJG2) flowing normally, but `msh/US/2/e/CentralNJ/#` sat completely silent for 3+ min live tests despite `sjmesh-relay.service` restarting cleanly and immediately blocking one bad-node packet (`!699a9390`) on reconnect — no new legitimate traffic seen since. Historical mosquitto log confirms relay was working normally as recently as Aug 17 ~23:36 EDT (last relayed packet), so this is a recent-onset gap, not long-standing. Overnight unattended monitor running: `nohup mosquitto_sub -h localhost -u meshdev -P [REDACTED - broker password, scrubbed Aug 30 2026] -t 'msh/US/2/e/CentralNJ/#' -v > /tmp/centralnj_overnight.log 2>&1 &` (PID 441186 as of Aug 18 18:4x EDT). **Next session: check with `wc -l /tmp/centralnj_overnight.log` and `tail -50 /tmp/centralnj_overnight.log`** — if populated, was just a quiet window; if still empty after a full overnight period, escalate to inspecting `sjmesh-relay.py`/SJMesh broker-side more deeply.
...
- **[NEW Aug 18]** Malla/meshtastic_history.db schema unknown — guessed table name `packet_history` at path `/opt/stacks/mqtt/meshtastic_history.db` does NOT exist (confirmed via error). Correct path/table/column names not yet found — needed for any future direct DB querying of Meshtastic packet history (e.g. to get last-seen timestamps per channel without relying on live `mosquitto_sub` sampling).
