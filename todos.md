...
- **[NEW Aug 23]** Once the LoRa APRS tracker board (K2GIA-9, on order) arrives and is configured, Charles considers active RF projects "done" aside from break/fix — worth a light doc consolidation pass at that point given the repo has grown large. Also pending: Charles plans to upload an Icom IC-2730A user manual for Claude to parse into a reference doc (`docs/icom-2730-reference.md`) for future troubleshooting help — not yet uploaded as of this date.
- **[NEW Aug 23]** K2GIA-7's KISSLink APRS-IS Companion tab shows Disconnected despite correctly-filled-in fields — not yet root-caused. See `known-issues.md` and `docs/aprs-bulletins-and-bridging-reference.md`. Next step: check Basic/Verbose log on that tab for a connection error when revisited.
