...
- **meshcore-mqtt-bridge can't self-heal after a broker restart — REAL FIX NEEDED.** Root cause found in the July 25 MeshCore audit: when Mosquitto on cnjmesh1 restarted, the bridge on cnjmesh3 kept running but went deaf, filled its 1000-message queue (`MQTT: stopped | Queue: 1000/1000`, `Timeout queueing message`), and never auto-reconnected. Manual `docker restart meshcore-mqtt-bridge` cleared it, but this WILL recur on every broker restart/reboot/crash. To actually fix: (1) confirm the container has `restart: unless-stopped` (helps only if it dies, which it didn't); (2) check for MQTT reconnect/keepalive settings in `/opt/meshcore-mqtt/.env.docker` or whether `meshcore-mqtt:local` lacks auto-reconnect; (3) build a watchdog (same pattern as the CoreScope ingest-stall and graywolf watchdogs) that detects "MQTT: stopped" / full queue and restarts the bridge. This is a silent-running-but-not-delivering failure — exactly the class the existing watchdogs cover.
- **Verify cnjmesh3 hardware/host device mapping** — the July 25 audit lists `/dev/ttyACM0` = Heltec V4 (observer) and `/dev/ttyACM1` = RAK4631 (companion/hub). Standing notes have it REVERSED: Observer = RAK4631 on ttyACM0, KPR2 = Heltec V4 on ttyACM1. Confirm reality with `ls -l /dev/serial/by-id/` on cnjmesh3 before trusting either. Same renumbering-risk class as the July 25 Graywolf ttyUSB issue.
