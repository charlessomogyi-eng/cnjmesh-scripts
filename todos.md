...
- **[RECHECK next session] CoreScope `local` source + Observer feed health.** During the July 26 meshomatic fix, the Observers page showed the `local` source (tcp://172.17.0.1:1883) as **disconnected** with ~1 pkt/5m, and the CNJ Mesh Observer row reading "last packet 6d ago / 0/hr" with a packet-health warning — BUT this was ~3 min after a CoreScope restart, so the counters are likely restart artifacts, not a real outage. **Action:** let CoreScope run 10+ min, reload the Observers page. If `local` shows connected with the 5m count climbing → it was just settling, ignore. If `local` is STILL disconnected / Observer still 0/hr after clean uptime → it's real: chase the Observer→cnjmesh3 bridge→cnjmesh1 Mosquitto→CoreScope chain (note the recurring `pingresp not received` local-MQTT keepalive flapping seen in corescope logs — same broker-fragility theme as the meshcore-mqtt-bridge). Consider extending the CoreScope watchdog to also check `local` source health / a per-source freshness alert (see monitor idea below).
