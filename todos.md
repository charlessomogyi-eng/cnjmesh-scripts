...
- **[NEW Aug 27]** Switch K2GIA-10's `stationMode` from `2` (Rx+Tx iGate) to `3` (Digipeater simplex) — Charles's decision: keep digipeating for the local LoRa mesh, stop gating packets to APRS-IS entirely. Reasoning: a 2m station (K2GIA-1) already covers IS access, and once a packet's on IS it's indistinguishable from any other APRS packet regardless of origin — so a second device also gating to IS adds nothing for Charles's own traffic. Trade-off understood and accepted: this means K2GIA-9's contacts from Aug 26/27 (KC2BPP-1, N2YDC-1, N2YDC-5 — all reached via IS/`TCPIP` in their paths) would not be reachable this way going forward; only genuine RF-range contacts would work after the switch.
  - Needs K2GIA-10's WebUI (`stationMode` field) — reachable via its own onboard WiFi (LilyGO T3 V1.6.1 has built-in ESP32 WiFi, independent of its USB-to-cnjmesh1 power connection). Find K2GIA-10's IP via router client list on `C4Somogyi-24`, or it may still be broadcasting its own config AP if not currently WiFi-connected.
  - After the switch, K2GIA-10 no longer needs WiFi at all for ongoing operation (digipeating is pure RF) — WiFi is only needed for this one-time config visit.
  - Note: may interact with the existing lora-aprs.live syslog todo above — unconfirmed whether syslog event reporting still functions in Mode 3 (digipeater-only) or was tied specifically to iGate mode. Check before relying on it post-switch.
