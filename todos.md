...
- **Rotate `meshuser`/`large4cats` on your own `mqtt.cnjmesh.me`** — distinct from SJMesh's own `meshuser` credential (not yours to rotate); this one is genuinely yours and still pending.
