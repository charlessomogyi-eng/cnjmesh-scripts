...
**Known gaps, not yet covered by any layer:** none — `mesh-discord-shim` (Docker, **MeshCore**-specific — advertisement/channel-message events from the Observer, `a8c40bf3` — to Discord) got its own watchdog Sep 2, 2026 (`watchdogs/mesh-discord-shim-watchdog/`, container-state + log-freshness checks, not yet deployed to cnjmesh1 — see below). `graywolf-discord-bridge` and `graywolf.service` itself both already have working watchdogs (`graywolf-discord-watchdog.timer` — auto-restarts; `graywolf-watchdog.timer` — alert-only, no auto-restart due to PTT risk); both committed to git under `watchdogs/`. `aprs_monitor.py`'s 48hr dead-air check is CLOSED/rejected (Sep 2, 2026) — APRS traffic volume isn't a reliable health signal, so this stays permanently disabled; `graywolf-watchdog` (service-up check) is sufficient. CoreScope's `local` source instability is unmonitored (root cause is known, see `todos.md` and session-log Aug 4 entry — no auto-heal built yet). See `todos.md` for current priority.
