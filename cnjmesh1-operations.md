...
**Known gaps, not yet covered by any layer:** `mesh-discord-shim` (Docker, new-node Discord welcome messages) has no watchdog — genuine gap. `graywolf-discord-bridge` and `graywolf.service` itself both already have working watchdogs (`graywolf-discord-watchdog.timer` — auto-restarts; `graywolf-watchdog.timer` — alert-only, no auto-restart due to PTT risk); both committed to git under `watchdogs/`. `aprs_monitor.py`'s 48hr dead-air/crash checker remains intentionally disabled — Charles confirmed he only needs "is Graywolf down," which `graywolf-watchdog` already covers. CoreScope's `local` source instability is unmonitored (root cause is known, see `todos.md` and session-log Aug 4 entry — no auto-heal built yet). See `todos.md` for current priority.
