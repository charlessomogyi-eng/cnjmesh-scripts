...
- **K2GIA-9 (LoRa APRS Tracker, T3 V1.6.1) has no physical user button populated.** The button is required to reopen the WiFi config AP (`LoRaTracker-AP`) after first flash — without it, changing ANY setting later (callsign, SmartBeaconing, frequency) requires a full re-flash from scratch. Install the button BEFORE or IMMEDIATELY AFTER first flash, not later. Workaround if not yet soldered: briefly touch IO15 to GND three times (jumper/tweezers/paperclip) — same effect as triple-pressing the button. Full install instructions: `docs/lora-aprs-reference.md`.
