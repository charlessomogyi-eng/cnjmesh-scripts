...
and the graywolf watchdogs) when a Pi's root filesystem or CPU temperature
crosses a threshold. Alert-only on state change, matching the existing
watchdog pattern -- no repeated spam every 5 minutes.
