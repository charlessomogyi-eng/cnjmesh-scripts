...
**HARD RULE: Charles never runs `git add`/`commit`/`push` himself, and never wants to.** He pastes command output (from Pi terminals) into chat, and the AI pushes those changes directly to GitHub using a token he provides in that session — that's the entire workflow, permanently. Do NOT set up SSH deploy keys, credential caching, or any other git-auth mechanism on cnjmesh1/2/3 to let Charles push locally — he doesn't want that ability, has never wanted it, and doing so was a wasted-effort mistake on July 26, 2026 that had to be fully reverted on both cnjmesh1 and cnjmesh3 (leaving harmless unused SSH keys behind on both hosts and two unused entries in his GitHub SSH keys list — safe to ignore or clean up, not functionally relevant). The only git command Charles ever runs locally on a Pi is `git pull`, to refresh that Pi's on-disk copy of a doc (e.g. `CLAUDE_CONTEXT.md`) after the AI has pushed a change — and HTTPS with no saved credential is fine for that, since a pull with no local changes doesn't require authentication against a public repo. If the AI needs to author or commit anything, it does so from its own sandboxed clone using a token pasted by Charles, never by asking him to run git commands himself beyond an occasional `git pull`.
