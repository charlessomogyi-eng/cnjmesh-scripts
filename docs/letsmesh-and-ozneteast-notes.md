...
- Authentication is often automatic via the node's own MeshCore auth token —
  **no shared username/password needs to be distributed**, unlike our current
  `meshdev`/`[REDACTED - broker password, scrubbed Aug 30 2026]` setup
...
- **Auth:** JWT/token-based, tied to your node's own public key — **not** a shared username/password like `meshdev`/`[REDACTED - broker password, scrubbed Aug 30 2026]`. The bridge software (meshcore-packet-capture or meshcoretomqtt) generates/signs this automatically; may require installing `meshcore-decoder` (npm package) on cnjmesh1 if not already present — check before assuming this is a zero-install task.
