...
Container: `meshcore-mqtt-bridge` (image `meshcore-mqtt:local`, a local build — NOT the public ghcr image). USB device: `/dev/serial/by-id/usb-Espressif_Systems_heltec_wifi_lora_32_v4_..._E8F60AC9DEB4-if00`. Credentials `meshdev`/`[REDACTED - broker password, scrubbed Aug 30 2026]`.
